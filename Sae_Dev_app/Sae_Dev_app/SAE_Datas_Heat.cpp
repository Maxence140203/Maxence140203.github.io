...
Arbre SAE_Datas_Heat::calculDeltaT(size_t mois, const std::vector<float>& vTmoyStation, const std::vector<float>& vTSigmaStation)
{
    ...
    for (size_t i = mois; i < vTmoyStation.size(); i += 12)
    {
        CelluleArbre newCel;
        float deltaT = 0;
        if (i + 12 < 360)
        {
            newCel._tempMois = vTmoyStation[i];
            newCel._tSigmaMois = vTSigmaStation[i];
            newCel._deltaT = vTmoyStation[i + 12] - vTmoyStation[i];
            newCel._annee = annee + int(i / 12);
        }
        else
        {
            newCel._tempMois = vTmoyStation[i];
            newCel._tSigmaMois = vTSigmaStation[i];
            newCel._deltaT = vTmoyStation[i];
            newCel._annee = annee + int(i / 12);
        }
        a.ajoute_cellule(newCel);
    }
    return a;
}
...
std::vector<float> SAE_Datas_Heat::GetTMoyChaqueMois() //_vRawData[0] est pour le premier fichier _vRawData[0][0] est pour la première station et le .first est pour récupérer la température moyenne TMoy
{
	std::vector<float> vTMoyChaqueAnnee;
    for (int i = 0; i < nbMonths(); i++)
	{
        float total = 0;
		for (const auto& [IDstation, tMoySigma] : _vRawData[i])
		{
			total += tMoySigma.first;
		}
        total /= _vRawData[0].size();
		vTMoyChaqueAnnee.push_back(total);
    }
    return vTMoyChaqueAnnee;
}

std::vector<float> SAE_Datas_Heat::GetTMinChaqueMois()
{
    std::vector<float> vTMin;
	float min = 100;
    for (int i = 0; i < nbMonths(); i++)
    {
        for (const auto& [IDstation, tMoySigma] : _vRawData[i])
        {
            if (tMoySigma.first < min)
            {
                min = tMoySigma.first;
            }
        }
...
std::vector<float> SAE_Datas_Heat::GetTMaxChaqueMois()
{
    std::vector<float> vTMax;
    float max = -100;
    for (int i = 0; i < nbMonths(); i++)
    {
        for (const auto& [IDstation, tMoySigma] : _vRawData[i])
        {
            if (tMoySigma.first > max)
            {
                max = tMoySigma.first;
            }
        }
